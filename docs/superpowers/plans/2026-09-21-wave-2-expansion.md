...
```
Stub returns `(0, nil)` (first fails). README: `strconv.Atoi`/`Itoa`, string/number boundary.
...
defer func() { Now = old }()
if !IsExpired(time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)) { t.Fatal("past expiry not expired") }
if IsExpired(time.Date(2026, 1, 3, 0, 0, 0, 0, time.UTC)) { t.Fatal("future expiry reported expired") }
```
Stub (`expiry.go`):
```go
